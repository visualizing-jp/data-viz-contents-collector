...
export default async function ContentsPage({
  ...
  searchParams: Promise<{ q?: string; publisherId?: string; page?: string }>;
}) {
  const { q, publisherId, page } = await searchParams;
      ...
      {rows.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 p-12 text-center text-gray-500">
          ...
          <p className="mt-1 text-sm">
            <Link href="/sources" className="text-blue-600 hover:underline">ソースを登録</Link>
            して収集を開始してください
          </p>
        </div>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="mt-8 flex justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={`/contents?${new URLSearchParams({ ...(q ? { q } : {}), ...(publisherId ? { publisherId } : {}), page: String(p) })}`}
